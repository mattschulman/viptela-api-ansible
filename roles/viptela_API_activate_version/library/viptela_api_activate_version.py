...
short_description: A module to generate a HTTP POST Body and then send the POST to a vManage host to start an activate job.
version_added: "0.0.1"
description: A module to generate a HTTP POST Body and then send the POST to a vManage host to start an activate job.
options:
    vmanage:
...
class ViptelaAPI(object):
  ...
  def softwareActivate(self):
    ''' Connect to vManage and activate an inamge on a vEdge'''
    ...
    url = 'https://' + self.vmanage + '/dataservice/device/action/changepartition'
    ...
    for device in self.device_list:
      ...
      payloadDict['devices'].append(activateDeviceDict)
    ...
    self.activateID = ansi_response['id']
    self.print_debug("  -- activateID: ", self.activateID, 1)
...
def main():
  ...
  viptelaSession.print_debug("-- viptela-softwareActivate: ", "", 1)
  if module.check_mode:
    module.exit_json(**viptelaSession.result)
  
   # do action + output structure enrichment and evaluation
  if viptelaSession.softwareActivate():
    viptelaSession.result["last_status"] = viptelaSession.activateID
    viptelaSession.result['changed'] = True       
  else:
    viptelaSession.result["stdout_lines"] = viptelaSession.activateID
